...
void loop() {
  ...
  if (stateMachine == 1) {
    ...
    // desinfection();
    delay(250);  //maybe need to increase timing
  ...
  checkHuman3(humanSense1, humanSense2, humanSense3);
...
int checkHuman3(bool a, bool b, bool c) {
  // int k = m;
  if (a == true || c == true) {  //Current pod uses 2 sensors
    m += 1;
    if (m > 65 && (a == true || c == true)) {
      stateMachine = 1;
      humanState = HIGH;
      relay1State = LOW;   //Ventilation Fan             ON
      relay2State = LOW;   //Air con                     ON
    ...
    humanState = LOW;
  }return m;      //not sure how to return the m counter to the serial print
}
